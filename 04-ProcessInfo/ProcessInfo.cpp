    ...
    PDWORD pPolicy, PDWORD pResourceIntegrityLevel, PDWORD pResourcePolicy)
{
    ...
    *pResourcePolicy = 0;
    ...
    if (hToken != NULL)
    {
        dwResult =GetSecurityInfo(hProcess, SE_KERNEL_OBJECT, LABEL_SECURITY_INFORMATION, NULL, NULL, NULL, &pSACL, &pSD);
...
BOOL GetProcessIntegrityLevel(DWORD PID, PDWORD pIntegrityLevel, PDWORD pPolicy, PDWORD pResourceIntegrityLevel, PDWORD pResourcePolicy)
{
    ...
    HANDLE hProcess = OpenProcess(READ_CONTROL | PROCESS_QUERY_INFORMATION, FALSE, PID);
    ...
    BOOL bReturn = GetProcessIntegrityLevel(hProcess, pIntegrityLevel, pPolicy, pResourceIntegrityLevel, pResourcePolicy);
...
VOID Dlg_PopulateProcessList(HWND hwnd)
{
    ...
    for (; fOk; fOk = thProcesses.ProcessNext(&pe))
    {
        ...
        if (GetProcessIntegrityLevel(pe.th32ProcessID, &dwCodeIntegrityLevel, &dwCodePolicy, &dwResourceIntegrityLevel, &dwResourcePolicy))
        {
            ...
            else
            {
                ...
                {
                    if ((dwCodePolicy & TOKEN_MANDATORY_POLICY_NO_WRITE_UP) == TOKEN_MANDATORY_POLICY_NO_WRITE_UP)
                    {
                        _tcscat_s(szCodeDetails, _countof(szCodeDetails), TEXT(" + no write-up"));
                    }

                    if ((dwCodePolicy & TOKEN_MANDATORY_POLICY_NEW_PROCESS_MIN) == TOKEN_MANDATORY_POLICY_NEW_PROCESS_MIN)
                    {
            ...
            else
            {
                ...
                {
                    if ((dwResourcePolicy & SYSTEM_MANDATORY_LABEL_NO_WRITE_UP) == SYSTEM_MANDATORY_LABEL_NO_WRITE_UP)
                    {
                        _tcscat_s(szResourceDetails, _countof(szResourceDetails), TEXT(" + no write-up"));
                    }

                    if ((dwResourcePolicy & SYSTEM_MANDATORY_LABEL_NO_READ_UP) == SYSTEM_MANDATORY_LABEL_NO_READ_UP)
                    {
                        _tcscat_s(szResourceDetails, _countof(szResourceDetails), TEXT(" + no read-up"));
                    }
                    if ((dwResourcePolicy & SYSTEM_MANDATORY_LABEL_NO_EXECUTE_UP) == SYSTEM_MANDATORY_LABEL_NO_EXECUTE_UP)
                    {
                        _tcscat_s(szResourceDetails, _countof(szResourceDetails), TEXT(" + no execute-up"));
                    }
                }
            }
        }

        StringCchPrintf(sz, _countof(sz), TEXT("%s     (0x%08X)  %s    [%s]"), pszExeFile, pe.th32ProcessID, szCodeDetails, szResourceDetails);
...
VOID FormatSizeInKB(DWORD dwSize, DWORD nCharacters, LPTSTR szSize, size_t cchSize)
{
    ...
    if (StrFormatKBSize(dwSize, szFormattedSize, _countof(szFormattedSize)) == NULL)
    {
    ...
    {
        DWORD current = 0;
        for (current = 0; current < (nCharacters - _tcslen(szFormattedSize)); current++)
        {
            szSize[current] = TEXT(' ');
        }
...
VOID ShowProcessInfo(HWND hwnd, DWORD dwProcessID)
{
    ...
    for (; fOk; fOk = th.ProcessNext(&pe))
    {
        if (pe.th32ProcessID == dwProcessID)
        {
            ...
            AddText(hwnd, TEXT("   PID=%08X, ParentPID=%08X, PriorityClass=%d, Threads=%d, Heaps=%d\r\n"),
    ...
    AddText(hwnd, TEXT("\r\nModules Information:\r\n  Usage  %-*s(%-*s)  %10s  Module\r\n"),
            s_cchAddress, TEXT("BaseAddr"), s_cchAddress, TEXT("ImagAddr"), TEXT("Size"));
...
BOOL Dlg_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
    ...
    if (GetProcessElevation(&s_elevationType, &s_bIsAdmin))
    {
        ...
        GetWindowText(hwnd, _tcschr(szTitle, TEXT('\0')), _countof(szTitle) - _tcslen(szTitle));
        ...
        {
            Button_SetElevationRequiredState(GetDlgItem(hwnd, IDC_BTN_SYSTEM_PROCESSES), !bCanReadSystemProcesses);
        }
        else
        {
            ...
            MoveWindow(GetDlgItem(hwnd, IDC_BTN_SYSTEM_PROCESSES), 0, 0, 0, 0, FALSE);
        }
    }
...
BOOL Dlg_OnSize(HWND hwnd, UINT state, int cx, int cy)
{
    ...
    SetWindowPos(hwndCtl, NULL, n + btnRect.right, n, cx - n - n - btnRect.right, rc.bottom, SWP_NOZORDER);
...
void Dlg_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
    ...
    case ID_VMMAP:
    {
        ...
        DWORD dwStatus = StartElevatedProcess(TEXT("\"14-VMMap.exe\""), szCmdLine);
