...
void CheckInstances()
{
    // Create the boundary descriptor - a string identifier to name the scope where the private namespace will be defined
    ...
    //D: = Deny Access to everyone we don't explicitly allow;  A = Allow, GA = Generic All rights, BA = Buildin Administrators;  
    ...
    // A private namespace is just a directory where you create kernel bojects which has an associated security descriptor
    // Kernel objects created with a prefix based on a private namespace appear in Process Explorer with a "...\" prefix.
    ...
    int       nCmdShow)
{
    ...
    if (g_hNamespace != NULL)
    {
        ...
        {  
            // Open namespace, leave it visible after we close it
            ClosePrivateNamespace(g_hNamespace, 0);
        }
        else
        { 
            // Created namespace, pass flag so it isn't visible after we close it
    ...
    if (g_hBoundary != NULL)
    {
        // NOTE: The object returend by CreateBounaryDescriptor isn't a HANDLE, so we don't use CloseHandle() to destroy it
